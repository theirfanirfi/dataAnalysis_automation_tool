...
def leftColumn(tk, main_frame, data):
    ...
    #buttons
    #separate files are made for the buttons, where the design and event listeners are located.
    ...
    # sub-container in the left column
    sub_container = tk.Frame(left_column, bg="lightgreen")
    sub_container.grid(row=6, column=0, columnspan=2, sticky='w')

    # first sub-column in the sub-container
    #input fields for setting smoothing factor and membrane area
    smoothingFactor(tk, sub_container, data)
    membrane_area(tk, sub_container, data)

    # second sub-column in the sub-container
    #currenlty not used, because everything was handled in the header, statistical and plot areas
    ...
    #entire leftcolumn is set to the state (Data class) for event driven
